...
{
    function change_genres(items) {
        change_state_value("genres", items )
    }

    function select_genres(id) 
    {
        let genres_list = genres.slice();
        list.find(item =>
        {
            if(item.id === id && genres.length < 3 )  genres_list.push(item)
            return false
        })
        change_genres(genres_list)
    }

    function remove_genres(id) 
    {
    ...
    return(
    <div className="create-book-element__genres-element">
        <span className="create-book-element__genres-title">Géneros  ( 3 maximo )</span>
        <GenresSelected items={genres}/>
        <div className="create-book-element__genres-list">
            {list.map((item, i) =>
            {
                let s = false;
                ...
                return <CheckButton select={select_genres} remove={remove_genres} selected={s} genres={genres} text={item.tag} id={i + 1}  key={i + 1} />
            })}
        </div>
    </div>)    
}
